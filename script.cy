...
CREATE (airport:Airport { airportId: toInteger(csvline.AirportID),
 ...
 DST:csvline.DST,ICAO:csvline.ICAO,IATA:csvline.IATA, source:csvline.Source, 
 type:csvline.Type, tz:csvline.Tz, latitude:csvline.Latitude, 
 longitude:csvline.Longitude, altitude:csvline.Altitude })
 
load csv with headers from "file:///airlines.csv" as csvline 
CREATE (airline:Airline { airlineId: toInteger(csvline.AirlineID), 
country: csvline.Country,ICAO:csvline.ICAO,IATA:csvline.IATA, 
name:csvline.Name, alias:csvline.Alias, callsing:csvline.Callsign, 
active:csvline.Active})

load csv with headers from "file:///routes.csv" as csvline 
CREATE (route:Route { airLine:csvline.Airline, 
airlineId:toInteger(csvline.AirlineID),source:csvline.Source,
sourceID:toInteger(csvline.SourceID), destination:csvline.Destination, 
destinationId:toInteger(csvline.DestinationID),codeshare:csvline.Codeshare, 
stops:csvline.Stops,equipment:csvline.Equipment})

load csv with headers from "file:///airports.csv" as csvline
CREATE (city:City { cityId: toInteger(csvline.AirportID), 
country: csvline.Country,cityName:csvline.City,airportName:csvline.Name, 
timezone:csvline.Tz})
